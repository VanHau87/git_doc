## 0. Config name và email cho git với `git config`
...
```bash
git config --global user.name "Nguyen Van Hau"
git config --global user.email nguyenvanh29@gmail.com
```
...
## 2. Hiển thị trạng thái với `git status`
...
## 3 Các khu vực làm việc với Git

Chúng ta sẽ có các khu vực theo thứ tự dưới đây

1. **Khu vực làm việc**: Chính là nơi chúng ta đang code, vẫn ở local
2. **Khu vực staging**: Sau khi dùng `git add` (<> git reset) thì file sẽ được đưa lên khu vực này, vẫn ở trên local
